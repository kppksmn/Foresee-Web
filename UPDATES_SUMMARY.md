...
### 1. แก้ไขบั๊กการกดปิด Submenu ใน Sidebar 2 ครั้งหลัง Refresh หน้าจอ
- `src/layouts/AdminLayout/AdminLayout.tsx`:
  ...
  - ปัญหาก่อนหน้านี้เกิดจากเมื่อ Refresh หน้าจอ สถานะใน `openSubmenus[text]` จะมีค่าเป็น `undefined` (ซึ่งแสดงผลเปิด `isOpen = true`) แต่เมื่อกดคลิกครั้งแรก `!undefined` จะถูกประเมินเป็น `true` ส่งผลให้สถานะยังคงเป็นเปิดอยู่ (ต้องกดครั้งที่สองจึงกลายเป็น `false`)
  - หลังการแก้ไข: เมื่อคลิกครั้งแรก ระบบจะสลับจาก `undefined` (เปิด) เป็น `false` (ปิด) ได้ทันทีตั้งแต่การกดคลิกครั้งแรก 100%

### 2. บังคับใช้สิทธิ์ใช้งานในหน้าประเภทรถ (Vehicle Types Permission Enforcements)
...
### 3. เพิ่มตัวเลือกกรองวันที่นัดหมายในงานปัจจุบัน และประวัติงาน (Scheduled Date Filter for Jobs)
...
### 4. ลบข้อความตัวอย่าง `(ดดมมปปปป)` ในระบบรีเซ็ตรหัสผ่านพนักงาน
...
### 5. ล็อกและป้องกันการเลือกคนขับที่ใบขับขี่หมดอายุ (Driver License Expiration UI Guard)
...
### 6. ถอด `nameEn` ออกจากส่วนการแสดงผล 100%
...
### 7. ลบโมดูลและเมนูแผนที่ติดตาม (`/map`)
- `src/app/router/AppRouter.tsx`: ถอด Route `/map`
- `src/features/map`: ลบโฟลเดอร์คอมโพเนนต์

### 8. เพิ่มหน้าหลัก Home Page & ระบบกรองสิทธิ์ไดนามิก
...
### 9. ปรับเปลี่ยนไอคอนเมนูโฟลเดอร์บน Sidebar Navigation
